...
func GetAllPekerjaan() ([]model.PekerjaanAlumni, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&p.StatusPekerjaan, &p.Deskripsi, &p.CreatedAt, &p.UpdatedAt,
		)
		if err != nil {
			return nil, err
		}
...
func GetPekerjaanByID(id string) (model.PekerjaanAlumni, error) {
	...
	err := database.DB.QueryRow(
		"SELECT * FROM pekerjaan_alumni WHERE id=$1", id,
	).Scan(
		&p.ID, &p.AlumniID, &p.NamaPerusahaan, &p.PosisiJabatan, &p.BidangIndustri,
		&p.LokasiKerja, &p.GajiRange, &p.TanggalMulai, &p.TanggalSelesai,
		&p.StatusPekerjaan, &p.Deskripsi, &p.CreatedAt, &p.UpdatedAt,
...
func UpdatePekerjaan(id string, p model.PekerjaanAlumni) error {
	_, err := database.DB.Exec(
        ...
        SET alumni_id=$1, nama_perusahaan=$2, posisi_jabatan=$3, bidang_industri=$4, lokasi_kerja=$5, 
            gaji_range=$6, tanggal_mulai_kerja=$7, tanggal_selesai_kerja=$8, 
